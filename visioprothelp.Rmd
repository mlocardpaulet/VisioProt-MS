...
If you have an issue with the application, and want to report a bug, please do it [here](https://github.com/mlocardpaulet/VisioProt-MS/issues/new). 
...
[Download example](./files/Unique/Rowinpro_MS_OEJPA170302_93_Job_1_results.csv).

The table must have 4 tab-delimited columns without headers. These should include:

- "RT" in first position (we assume that this is in minutes)
- "Mass" in second position (we assume that this is in Da)
- "intensity" in third position 
...
[Download example](./files/test/Bruker_file_Std_Orexin_1-100_Slot2-46_01_1147_DeconvolutedPeaks.csv).

This file must contain the following information:

  + The first column (in line 2 of the file) should be "Compound Name"
  + "RT / min"
  + "MW"
  + "Intensity"
...
The file must start with "#TopFD".
...
The table must contain a column with the header "MinElutionTime".
...
TODO
<br />
This button is not accessible when visualizing RT ranges and not points (BioPharma Finder and ProMex exports). 
<br />

</div>
<div>

This button sums all point intensities in the plot area. It returns the following values:

- `Sum_intensities` = sum of intensities of the points in the plot area
- `Prop_sum_intensities` = `Sum_intensities` / sum of intensities of all the points in the plot (including outside of the area) with a `Percentage_signal_threshold` ≥ the one selected when pressing the button.
- `Point_count` = number of points in the plot area
- `Mass_min` = minimum mass included in the calculation
- `Mass_max` = maximum mass included in the calculation
- `RT_min` = minimum RT included in the calculation
- `RT_max` = maximum RT included in the calculation
- `Percentage_signal_threshold` = value set in the `Threshold` field (percentage of top intensities plotted)
...
<br />
<br />
To plot MS/MS results from Proteome Discoverer, the user needs to upload two .txt files resulting from the same search. With this output, the points for the MS/MS data will have the MH+ mass, so we expect + ~1 Da when compared to the MS trace. The PrSM IDs will be a concatenation of the protein description and the modification(s), unless the column "Modifications" is missing from the PrSM table.
...
This file must contain the following information:
  ...
  + Retention time in a column "RT.in.min" or "R..min.".
  + "Precursor.MHplus.in.Da".
  + "Spectrum.File"

<br />

- <u>The corresponding PrSM file:</u>

[Download example](./files/MS2/OFJMX160905_30-(1)_PSMs.txt).

  + The protein or proteoform identifier in a column "Master.Protein.Descriptions" or "Protein.Accessions".
  + Precursor MH+ ("mz.in.Da").
  + "Spectrum.File"
...
[Download example](./files/test/MSPathFinder_OFJMX160905_30_IcTarget.tsv).


In these tables, the Protein ID should be in column 8 and the MS1 feature ID in column 15. And each MS1 feature should be matched to only one protein ID.

Th columns should be ordered as follows:
...
<div class="col2">
<br />
![](Figures/Download.PNG) # TO UPDATE
<br />
<br />
...
The button `Download table` exports a tab-delimited tsv file with the values for all points in the plot. 
</div>